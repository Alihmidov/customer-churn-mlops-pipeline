...
from sklearn.metrics import classification_report, confusion_matrix, roc_auc_score, DisplayLabels
...
def evaluate_model():
    try:
        ...
        data_path = "data/processed/engineered_churn_data.csv"
        
        if not os.path.exists(model_path):
            raise FileNotFoundError(f"Trained model not found at {model_path}. Run training first!")

        logger.info(f"Loading model from {model_path} and data from {data_path} for evaluation...")
        ...
        with mlflow.start_run(run_name="Model_Evaluation_Metrics") as run:
